...
class ScheduleAdjuster:
    ...
    def analyze_and_adjust(self) -> Dict[str, Any]:
        ...
        if adjustment["adjustment_needed"]:
            ...
            return {"adjusted": True, "adjustment": adjustment, "analysis": analysis}
    ...
    def _analyze_performance(self) -> Dict[str, Any]:
        ...
        plans = (
            self.db.query(DailyStudyPlan)
            .filter(DailyStudyPlan.schedule_id == self.schedule.id)
            .order_by(DailyStudyPlan.plan_date)
            .all()
        )
        ...
        adherence_rate = (
            (len(completed_days) / total_past_days * 100) if total_past_days > 0 else 0
        )
        ...
        catch_up_needed = self._calculate_catch_up_needed(
            missed_days, partially_completed
        )
    ...
    def _calculate_adjustment(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if analysis["is_overloaded"] or analysis["consecutive_missed"] >= 3:
            ...
            new_minutes = max(
                self.schedule.min_daily_load, current_daily_minutes - reduction
            )
        ...
        if analysis["catch_up_needed"]["days"] > 0:
            if self.schedule.catch_up_strategy == "skip":
                ...
                new_minutes = min(
                    self.schedule.max_daily_load, current_daily_minutes + increase
                )
                adjustment["adjustment_needed"] = True
                adjustment["new_daily_minutes"] = new_minutes
                adjustment["reason"] = (
                    f"Gradually catching up {catch_up_days} missed days"
                )
                return adjustment
                ...
                adjustment["new_daily_minutes"] = new_minutes
                adjustment["reason"] = (
                    f"Intensive catch-up for {catch_up_days} missed days"
                )
                return adjustment

        # Strategy 3: Increase load if doing very well
        if (
            analysis["adherence_rate"] >= 90
            and analysis["avg_performance"] >= 85
            and current_daily_minutes < self.schedule.max_daily_load
        ):
    ...
    def _count_consecutive_missed(
        self, plans: List[DailyStudyPlan], today: date
    ) -> int:
        """Count consecutive missed days (burnout indicator)"""
        missed_recent = [
            p for p in plans if p.plan_date < today and p.status == "skipped"
        ]
    ...
    def _calculate_avg_performance(
        self, completed_plans: List[DailyStudyPlan]
    ) -> float:
    ...
    def _calculate_catch_up_needed(
        ...
        return {
            ...
            "estimated_catch_up_days": max(
                1, int(total_missed / 2)
            ),  # Spread over half the days
        }
...
def adjust_schedule(db: Session, schedule_id: int, user_id: int) -> Dict[str, Any]:
    ...
    if not schedule:
        return {"adjusted": False, "error": "Schedule not found"}
