...
def get_schedule(db: Session, schedule_id: int, user_id: int) -> Optional[StudySchedule]:
    """Get a single study schedule by ID"""
    return db.query(StudySchedule).filter(
        and_(StudySchedule.id == schedule_id, StudySchedule.user_id == user_id)
    ).first()
...
def get_schedules(
    ...
    return query.order_by(StudySchedule.created_at.desc()).offset(skip).limit(limit).all()
...
def create_schedule(
    ...
    config_dict = schedule.schedule_config.model_dump() if hasattr(schedule.schedule_config, 'model_dump') else schedule.schedule_config
    milestones_dict = None
    if schedule.milestones:
        milestones_dict = [m.model_dump() if hasattr(m, 'model_dump') else m for m in schedule.milestones]
...
def update_schedule_stats(
    ...
    plans = db.query(DailyStudyPlan).filter(
        DailyStudyPlan.schedule_id == schedule_id
    ).all()
...
def get_schedule_plans(
    ...
    query = db.query(DailyStudyPlan).filter(
        DailyStudyPlan.schedule_id == schedule_id
    )
...
def get_upcoming_plans(
    ...
    return get_schedule_plans(db, schedule_id, user_id, start_date=today, end_date=end_date)
